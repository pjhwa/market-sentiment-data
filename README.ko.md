...
│
├── briefing/
│   ├── latest.json                  # 아침 브리핑 (schema_version 1.1) — 항상 최신
│   ├── briefing.log                 # 크론 로그
│   └── history/
│       └── YYYY-MM-DD.json
│
...
**TIER1 — 빅테크/대형주 (12종목): 개별 심층 분석, 하루 2회 (pre_open + post_close)**
...
이슈 설명 + 시장 영향 포함 `global_context` 섹션, 이중 언어 브리핑 텍스트, 핵심 테마를 반환합니다.
