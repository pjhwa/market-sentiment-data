...
11. PRICE BINDING — Every price figure you state for a ticker (in headline, leaders/laggards, watchlist, action calls, or prose) must be copied character-for-character from that ticker's row in the SniperBoard/API price table provided in this prompt. Do not recall, estimate, round, or recompute a price from training memory or from a different section's figure. If a ticker's price is not present in the table, omit the price rather than stating one.
""")
    return "\n".join(lines)
