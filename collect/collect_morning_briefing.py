...
7. SECTOR LEADERS ACCURACY — When listing sector/theme leaders, do not include a ticker whose SniperBoard market_structure/Stage is DOWNTREND without an explicit caveat (e.g., "despite DOWNTREND structure") directly beside it; 
8. TERMINOLOGY — On first use anywhere in the output of a technical/trading acronym or abbreviation (e.g., RS, ATR, VWAP, RSI, EMA), briefly spell out or define it inline (e.g., "RS(상대강도)"); do not use an unexplained abbreviation.
never present a DOWNTREND ticker as an unqualified leader.
