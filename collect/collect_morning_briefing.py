...
7b. MOOD/TAG CONSISTENCY — A ticker's mood/sentiment tag (e.g., optimistic/bullish) must not contradict a same-day price-moving catalyst (e.g., a premarket/intraday decline) that appears in the earnings block or in a global issue cited elsewhere in this briefing for that same ticker; if such a conflicting catalyst exists, the mood tag must reflect it or explicitly note the tension.
