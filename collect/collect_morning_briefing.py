...
2. CURRENT STATE BINDING — For any listed issue, do not contradict or soften Current State with older
   training recollection. Prefer the provided state, dates, and scope verbatim. Do NOT generalize a specific named actor, location, attack, or event given in the input into a vague term (e.g., do not write "regional tensions" when the input names a specific attack/entity/pipeline) — always use the most specific description available in Current State/Summary. Do NOT add causal or comparative framing (e.g., "undershot forecasts", "beat expectations", "reinforced cut bets") unless the provided Current State/Summary explicitly states both the actual figure and what it was compared against — if the evidence says a figure matched consensus, report it as in line, not as a beat/miss/undershoot, and do not name a data release as the primary driver of a market reaction unless the evidence explicitly says so. EVENT RECENCY — Before describing an issue as breaking, fresh, or same-day news, compare its Current State date/timing to this run's fetched_at timestamp; if the event occurred more than 24h before fetched_at, frame it as an ongoing/already-known development (e.g. "as previously reported", "continuing to weigh on markets") rather than implying it just happened. If Current State indicates an anticipated event (e.g., a rate decision, vote, or hearing) has already occurred, state that outcome as already delivered per Current State — do not describe it as still pending or 'to be decided,' even if training memory suggests otherwise. PRICE/DIRECTION FIDELITY — When citing a price level, percentage move, or day's direction (e.g., commodity/index closes, "rose"/"fell"), use only the exact figures and direction stated in Current State/Summary, and match the [DIRECTION] tag given for that issue — do not invert the reported direction, invent a price level/streak/two-week-low claim, or substitute a cause not stated in the evidence, even if a different direction or level seems more familiar from training memory.
...
4. MOOD-PRICE BINDING — The mood/sentiment label assigned to any ticker (e.g., optimistic/bullish) must not contradict that ticker's own price move for the period discussed; if a ticker's price is down (e.g., -3% or more), do not label its mood optimistic/bullish without explicitly reconciling the drop in the same sentence (e.g., a bullish thesis stated despite the pullback).
5. TERMINOLOGY — On first use of any technical abbreviation or jargon term (e.g., RS, ATR, VWAP), spell out or briefly define it in the same sentence; do not use undefined acronyms. This self-check is mandatory for every ticker named anywhere in the output — including a single mention in a watchlist, leaders/laggards list, or action-call line — do not skip it because the ticker seems minor or peripheral to the main narrative.
...
6b. CRITICAL-TIER COVERAGE — Every issue tagged [CRITICAL] in the list above must be mentioned at least briefly (one clause is enough) somewhere in big_picture.summary or executive_bullets, even when rule 4 limits the lead narrative to 1–2 sentences on the single highest-ranked issue. Do NOT let sentence-count limits or single-cause-narrative framing cause a [CRITICAL] issue to be silently omitted from the briefing.
...
7b. MOOD/TAG CONSISTENCY — A ticker's mood/sentiment tag (e.g., optimistic/bullish) must not contradict a same-day price-moving catalyst (e.g., a premarket/intraday decline) that appears in the earnings block or in a global issue cited elsewhere in this briefing for that same ticker; if such a conflicting catalyst exists, the mood tag must reflect it or explicitly note the tension. This also applies when grouping tickers into a sector/theme narrative (e.g., market mood summary): if a ticker's own session price move or structural label (per SniperBoard/watchlist data) contradicts the group's overall direction (e.g., grouped under 'growth names under pressure' while its own data shows it as the session's outlier gainer), explicitly flag that ticker's divergence in the text rather than silently including it in the contradicting group.
7c. SECTOR CAUSATION ACCURACY — Do not attribute a ticker's relative strength/weakness to a macro theme (e.g., "oil spike") unless that ticker has real, direct exposure to that theme (e.g., an oil/energy producer or heavy fuel-cost consumer); for tickers in unrelated sectors (e.g., nuclear/clean-energy utilities, AI/data-center infrastructure), attribute moves only to causes evidenced in the listed issues, earnings block, or SniperBoard data for that ticker/sector.
8. TICKER STRUCTURE BINDING — Every single time you write a ticker symbol anywhere in the output (leaders/laggards/watchlist/action calls, headline, and narrative sentences alike), format it as "TICKER (LABEL)" where LABEL is its exact market_structure/Stage value from the SniperBoard table verbatim (e.g., "TSM (ACCUMULATION)", "MU (DOWNTREND)"). This binding is MANDATORY for every ticker mentioned anywhere, including in sector/theme analysis prose and any ticker not part of a formal leaders/laggards list — omitting the structure tag for a listed ticker is a binding violation. This applies to EVERY ticker in the SniperBoard table without exception, including tickers you only mention once in passing — omitting the (LABEL) tag for any ticker is a critical error, not a stylistic choice. Never infer, recall from training memory, or carry over a structure label from a prior day; copy the value character-for-character from the SniperBoard table provided in THIS prompt, even if it contradicts what you expect the ticker's trend to be. A ticker symbol without "(LABEL)" immediately after it is a format error — there are no exceptions, including tickers only mentioned in prose. [SELF-CHECK] Before returning output, re-scan every line of your draft for ticker symbols and confirm each has a verbatim (LABEL) suffix — including ACCUMULATION, which is the label most often silently dropped; insert any missing label before finalizing. ACCUMULATION is the label most often silently dropped — treat it exactly like any other structure value (e.g., DOWNTREND) and never omit "(ACCUMULATION)" just because it seems like default/neutral state. BEFORE finalizing output, re-scan every ticker symbol you wrote (headline, leaders, laggards, watchlist, action calls, and prose) and confirm each has its "(LABEL)" — insert any missing one from the SniperBoard table before returning the result. If a ticker's label is DOWNTREND and it is being described as a leader/gainer, either drop it from that list or write it as "TICKER (DOWNTREND — despite structure)". Before finalizing output, re-scan every ticker you wrote — in every section (headline, leaders, laggards, watchlist, action calls, and prose) — and verify each individual occurrence carries its "(LABEL)" suffix; labeling a ticker correctly once does not exempt its other occurrences from this check. This check is MANDATORY for every ticker present in the SniperBoard table, not only ones you chose to discuss narratively — build a mental checklist of all SniperBoard tickers you referenced anywhere in the output and confirm each one carries its "(LABEL)" suffix at every occurrence before submitting; a ticker symbol appearing with zero "(LABEL)" tags anywhere in the output is a critical format error. Do NOT alter, soften, or invent a ticker's market_structure value to make it fit a narrative (e.g., writing UPTREND for a ticker so it can be called a leader when the table says DOWNTREND) — the label must always match the table exactly regardless of which section or narrative role the ticker is being used in; if the true label conflicts with the section's purpose, exclude the ticker or use the prescribed caveat sentence instead of falsifying the label.
...
11. PRICE BINDING — Every price figure you state for a ticker (in headline, leaders/laggards, watchlist, action calls, or prose) must be copied character-for-character from that ticker's row in the SniperBoard/API price table provided in this prompt. Do not recall, estimate, round, or recompute a price from training memory or from a different section's figure. If a ticker's price is not present in the table, omit the price rather than stating one. SELF-CHECK: Before finalizing output, for every price you wrote anywhere (headline, leaders/laggards, watchlist, action calls, prose), locate that exact ticker's row in the SniperBoard/API price table and confirm the digits match exactly — a price that differs from the table even by rounding (e.g. stating $744.1 when the table shows $777.59) is a binding violation; correct any mismatch before submitting.
""")
    return "\n".join(lines)
...
def validate_briefing(data: dict) -> bool:
    for field in ("headline_en", "headline_ko"):
        if not isinstance(data.get(field), str) or not data[field]:
            ...
            return False
    if len(data.get("headline_ko", "")) > 30:
        print(f"[WARN] headline_ko 길이 초과: {len(data['headline_ko'])}자", file=sys.stderr)
        return False
