...
- market_structure MANDATORY DISCLOSURE: if API data provides a market_structure classification for any ticker (e.g., ACCUMULATION, DISTRIBUTION, UPTREND, DOWNTREND), that exact classification MUST be explicitly stated in the ticker's briefing entry — omitting a provided market_structure label (e.g., API=ACCUMULATION yet no structure notation appears in the briefing entry) is a critical omission error.
