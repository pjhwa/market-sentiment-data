...
- big_picture.summary: incorporate the highest-ranked issue naturally (1 sentence); flag the market_paradox if present; if the session showed any major index (S&P 500, Nasdaq, Dow) declining ≥1% but no clear triggering event appears in the provided global context, prepend a WARNING line: 'Major session decline detected — triggering event absent from provided context; briefing may be incomplete'; market_mood composite_score and traffic_light must use the API-provided score verbatim — do not independently calculate or estimate the score; traffic_light must be derived from the API score value only using these exact numeric thresholds: green ≥65, yellow 45–64, red <45 (example: composite_score=60.7 → yellow, not green); furthermore, if any major index (S&P 500, Nasdaq, Dow) declined ≥1% in the same session, traffic_light MUST NOT be green regardless of composite_score — downgrade to yellow and note the index-score divergence; a traffic_light that requires a score ≥1.5 points higher than the API value to justify its color is a critical classification error
