...
├── collect/
│   ├── __init__.py
│   ├── collect_sentiment.py           # 수집기 1 — python -m collect.collect_sentiment
...
│   ├── latest.json               # 아침 브리핑: 항상 최신 (2단계 Grok 파이프라인)
│   ├── briefing.log              # 크론 로그
...
| `HERMES_CMD` | 자동탐색 (`shutil.which` → `~/.local/bin` → `/opt/homebrew/bin` → `/usr/local/bin`) | 모든 수집기 |
...
## 4. 수집기 1 — 소셜 심리 (`collect/collect_sentiment.py`)
...
**TIER1 — 빅테크/대형주 (12종목, 개별 심층 분석, 하루 2회):**
...
- `sentiment/latest.json` → 종목별 composite_score, sentiment, key_reason
...
### 출력 품질 가드레일 (2026-06-05)

일반적인 Grok 출력 오류에 대한 2중 방어:

**계층 1 — 프롬프트 규칙 (`build_brief_prompt()`의 `WRITING STYLE RULES`):**
- **규칙 6**: 도메인 간 인과/대조 연결어 금지. 3개 도메인: `crypto` (BTC/비트코인), `policy` (관세/tariff/제재), `rates` (금리/TNX/연준). 한국어 연결어(`~는데`, `~지만` 등)와 영어 연결어(`while`, `but`, `however` 등)는 직접 인과관계가 있을 경우를 제외하고 서로 다른 도메인 사이에서 금지. 관련 없는 이벤트는 "한편," / "Separately,"로 시작하는 별도 문장으로 분리.
- **규칙 7**: 문장당 하나의 도메인; `summary_ko` (30자 제한)는 단일 도메인.
- **규칙 8**: 모든 `_ko` 필드는 한글만 사용. 히라가나(あ…)와 가타카나(ア…) 엄격히 금지.

**계층 2 — 후처리 검증 + 재시도:**

| 함수 | 목적 |
|------|------|
| `validate_output_quality(data)` | 모든 텍스트 필드에서 위반 스캔; 위반 문자열 목록 반환 |
| `build_correction_prompt(prompt, violations)` | 원본 프롬프트에 위반 목록 + 수정 지침 추가해 재시도용 프롬프트 생성 |

`validate_output_quality()`는 두 가지 검사를 수행:
- **검사 A** (`_collect_all_fields`): 동일 텍스트에서 인과 연결어 + 2개 이상 도메인 키워드 감지
- **검사 B** (`_collect_ko_fields`): `_ko` 필드에서만 히라가나/가타카나(`[぀-ゟ゠-ヿ]`) 감지

**재시도 로직:** 위반 발견 시 → `build_correction_prompt()`로 수정 Grok 호출 1회. 수정 출력이 깨끗하면 원본 대체. 수정 실패 또는 여전히 위반이면 원본 저장 (품질 차단보다 데이터 완결성 우선).
...
**Grok 출력 스키마 (v2.0 — 이중 언어):**
```json
{
  "overall": {
    "summary_en": "한 문장 시장 요약 (영어, 60자 이내)",
    "summary_ko": "한 문장 시장 요약 (한국어, 40자 이내)",
    "bullets_en": ["신호 → 의미", "신호 → 의미", "신호 → 의미"],
...
**v1.x 하위 호환 필드** (`summary`, `bullets[]`, `text`)는 더 이상 생성되지 않지만 SniperBoard가 `tField()` 폴백으로 수용합니다.

---

## 8. 수집기 5 — 아침 브리핑 (`collect/collect_morning_briefing.py`)
...
- 범위: 무역/관세, 지정학, 중앙은행, AI 규제
- 출력: 3개 이슈 객체로 구성된 `global_context` 배열
...
- **`already_reported_possible` 플래그**: `_build_earnings_lookup`이 현재 KST 날짜 기준으로 `days_until`을 재계산(파일 생성 시각이 아님). `earnings_date == today_kst`인 종목은 이미 발표된 것으로 플래그 처리(미국 장 마감 후 ~KST 06:00, 브리핑 KST 06:45 이전).
- **BTC 급락 경보**: BTC 5D ≤ -10% 또는 1D ≤ -5%이면 `_format_macro_block`이 필수 `⚠⚠⚠ BTC CRASH ALERT` 블록을 주입해 Grok이 executive_bullets에 포함하도록 강제.
- **가격 오래됨 감지**: SniperBoard API가 `stage2.price_date`(마지막 봉 날짜)를 반환. `price_date < 이전 거래일`인 종목을 권한 테이블에서 플래그 처리.
- **52주 고점 수정**: 음수 `pct_from_52w_high` 값에 대해 올바른 부호 규칙으로 `high_52w = price / (1 + pct_from_52w_high/100)` 계산.
- **1일 등락 수정**: 마지막 2개 캔들에서 직접 계산 (이전에는 API 필드 누락으로 항상 0.00%였음).

### 데이터 소스

- `sentiment/latest.json` → 워치리스트 전체 복합 심리 + 무드
- SniperBoard `/daily` API → 가격, stage2 점수, RS, 기술적 신호 + `price_date`
- SniperBoard `/macro` API → BTC, VIX, 10Y, DXY, 원유
- `earnings/latest.json` → `EARNINGS_DATE_OVERRIDES` 적용된 예정 어닝
- Grok 실시간 웹 검색 (1단계 전용)

### 출력 스키마 (schema_version 1.1)
...
```json
{
  ...
  "briefing_en": "글로벌 거시 맥락과 워치리스트 심리를 결합한 오전 전망...",
...
|----------------------|----------|----------|
| `GET /api/sentiment` | `sentiment/latest.json` | 5~10분 |
| `GET /api/sentiment/history` | `sentiment/history/*.json` | 5분 |
